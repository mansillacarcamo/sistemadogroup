    ...
    if ($accion === 'crear' || $accion === 'editar') {
      ...
      if ($accion === 'crear') {
        ...
        $sqlUpd = "UPDATE estados_pago SET
          ...
          descripcion=?, monto_neto=?, iva=?, monto_total=?, fecha_vencimiento=?, fecha_pago=?,
          estado=?, forma_pago=?, observaciones=?, oc_id=?, cot_id=?";
        $paramsUpd = [
...
<div class="card shadow-sm mb-4">
  ...
  <div class="card-body">
    <form method="POST" action="estados_pago.php?tipo=<?= $tipoActivo ?>" class="row g-3">
      ...
      <!-- Adjuntar factura -->
      <div class="col-12">
        <label class="form-label fw-semibold"><i class="bi bi-paperclip me-1"></i>Factura / Comprobante</label>
        <?php $archActual=$registroEditar['factura_ruta']??''; $nomActual=$registroEditar['factura_nombre']??''; ?>
        <?php if ($archActual): ?>
        <div class="alert alert-success py-2 mb-2 d-flex align-items-center gap-2" style="font-size:.85rem">
          <i class="bi bi-file-earmark-check-fill"></i>
          <a href="uploads/<?= htmlspecialchars($archActual) ?>" target="_blank" class="fw-semibold">
            <?= htmlspecialchars($nomActual ?: basename($archActual)) ?>
          </a>
          <small class="text-muted">— ya adjunto</small>
        </div>
        <?php endif; ?>
        <input type="file" name="factura" class="form-control" accept=".pdf,.jpg,.jpeg,.png,.webp" onchange="prevFactura(this)">
        <div class="form-text">PDF, JPG o PNG · Máx 10MB</div>
        <img id="prevFacturaImg" src="" style="max-height:120px;border-radius:8px;border:1px solid #dee2e6;object-fit:contain;display:none;margin-top:6px">
      </div>
