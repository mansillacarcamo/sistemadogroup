...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  try {
    ...
    if ($accion === 'crear_jefe_obra') {
      ...
      $hash = password_hash($pass, PASSWORD_DEFAULT);
      $obraId = (int)($_POST['obra_id'] ?? 0) ?: null;
      $pdo->prepare("INSERT INTO jefes_obra (nombre,rut,email,telefono,obra_id,usuario,password_hash) VALUES (?,?,?,?,?,?,?)")
          ->execute([$nom, trim($_POST['rut'] ?? ''), trim($_POST['email'] ?? ''), trim($_POST['telefono'] ?? ''), $obraId, $usu, $hash]);
      $msg = "Jefe de obra '$nom' creado.";
    }
    if ($accion === 'editar_jefe_obra') {
      $id     = (int)$_POST['id'];
      $nom    = trim($_POST['nombre']   ?? '');
      $usu    = trim($_POST['usuario']  ?? '');
      $obraId = (int)($_POST['obra_id'] ?? 0) ?: null;
      $pdo->prepare("UPDATE jefes_obra SET nombre=?,rut=?,email=?,telefono=?,obra_id=?,usuario=? WHERE id=?")
          ->execute([$nom, trim($_POST['rut'] ?? ''), trim($_POST['email'] ?? ''), trim($_POST['telefono'] ?? ''), $obraId, $usu, $id]);
      if (!empty(trim($_POST['password'] ?? ''))) {
        $hash = password_hash(trim($_POST['password']), PASSWORD_DEFAULT);
        $pdo->prepare("UPDATE jefes_obra SET password_hash=? WHERE id=?")->execute([$hash, $id]);
      }
      $msg = "Jefe de obra '$nom' actualizado.";
    }
    if ($accion === 'toggle_jefe_obra') {
      $id = (int)$_POST['id'];
      $pdo->prepare("UPDATE jefes_obra SET activo = 1 - activo WHERE id=?")->execute([$id]);
      $msg = 'Estado actualizado.';
    }
    if ($accion === 'eliminar_jefe_obra') {
      $id = (int)$_POST['id'];
      $pdo->prepare("DELETE FROM jefes_obra WHERE id=?")->execute([$id]);
      $msg = 'Jefe de obra eliminado.';
    }
...
try {
  $jefesObra = $pdo->query("
    SELECT j.*, o.codigo AS obra_codigo, o.nombre AS obra_nombre
    FROM jefes_obra j
...
<div class="row g-4">
    ...
    <div class="card shadow-sm border-0">
      <div class="card-header bg-primary text-white d-flex align-items-center justify-content-between">
        ...
        <div class="d-flex gap-2">
          <a href="asignar_camion.php" class="btn btn-sm btn-warning text-dark fw-bold">
            <i class="bi bi-arrow-left-right me-1"></i>Asignar camiones
          </a>
  ...
  <!-- ── JEFES DE OBRA (segunda validación) ── -->
  <div class="col-12">
    <div class="card shadow-sm border-0 border-success" style="border-width:2px !important;">
      <div class="card-header text-white d-flex align-items-center justify-content-between" style="background:#15803d">
        <div>
          <h5 class="mb-0"><i class="bi bi-shield-fill-check me-2"></i>Jefes de Obra</h5>
          <small class="opacity-75">Segunda validación: confirman tickets ya validados por Carchek de su obra asignada</small>
        </div>
        <div class="d-flex gap-2">
          <a href="jefe_obra_login.php" class="btn btn-sm btn-outline-light" target="_blank"><i class="bi bi-box-arrow-in-right me-1"></i>Portal jefe obra</a>
          <button class="btn btn-sm btn-light" data-bs-toggle="modal" data-bs-target="#modalAddJefeObra"><i class="bi bi-plus-lg me-1"></i>Crear jefe</button>
        </div>
      </div>
      <div class="card-body p-0">
        <?php if (empty($jefesObra)): ?>
        <p class="text-center text-muted py-4 mb-0">No hay jefes de obra registrados.</p>
        <?php else: ?>
        <div class="table-responsive">
          <table class="table table-hover align-middle mb-0">
            <thead class="table-light">
              <tr><th>Nombre</th><th>RUT</th><th>Obra asignada</th><th>Correo</th><th>Teléfono</th><th>Usuario</th><th>Estado</th><th class="text-end">Acciones</th></tr>
            </thead>
            <tbody>
            <?php foreach ($jefesObra as $jo): ?>
            <tr class="<?= !$jo['activo'] ? 'text-muted opacity-60' : '' ?>">
              <td><strong><?= htmlspecialchars($jo['nombre']) ?></strong></td>
              <td><?= $jo['rut'] ? htmlspecialchars($jo['rut']) : '<span class="text-muted">—</span>' ?></td>
              <td>
                <?php if ($jo['obra_codigo']): ?>
                <small><strong><?= htmlspecialchars($jo['obra_codigo']) ?></strong> — <?= htmlspecialchars($jo['obra_nombre']) ?></small>
                <?php else: ?><span class="text-muted small">Todas las obras</span><?php endif; ?>
              </td>
              <td><small><?= $jo['email'] ? '<a href="mailto:'.htmlspecialchars($jo['email']).'">'.htmlspecialchars($jo['email']).'</a>' : '<span class="text-muted">—</span>' ?></small></td>
              <td><small><?= $jo['telefono'] ? htmlspecialchars($jo['telefono']) : '<span class="text-muted">—</span>' ?></small></td>
              <td><code><?= htmlspecialchars($jo['usuario']) ?></code></td>
              <td><?= $jo['activo'] ? '<span class="badge bg-success">Activo</span>' : '<span class="badge bg-secondary">Inactivo</span>' ?></td>
              <td class="text-end">
                <button class="btn btn-sm btn-outline-success" onclick="editJefeObra(<?= htmlspecialchars(json_encode($jo)) ?>)" title="Editar"><i class="bi bi-pencil"></i></button>
                <form method="POST" class="d-inline">
                  <input type="hidden" name="accion" value="toggle_jefe_obra">
                  <input type="hidden" name="id" value="<?= $jo['id'] ?>">
                  <button class="btn btn-sm <?= $jo['activo'] ? 'btn-outline-warning' : 'btn-outline-success' ?>" title="<?= $jo['activo'] ? 'Desactivar' : 'Activar' ?>"><i class="bi bi-<?= $jo['activo'] ? 'pause' : 'play' ?>-fill"></i></button>
                </form>
                <form method="POST" class="d-inline" onsubmit="return confirm('¿Eliminar este jefe de obra?')">
                  <input type="hidden" name="accion" value="eliminar_jefe_obra">
                  <input type="hidden" name="id" value="<?= $jo['id'] ?>">
                  <button class="btn btn-sm btn-outline-danger"><i class="bi bi-trash"></i></button>
                </form>
              </td>
        ...
        <div class="p-3 bg-light border-top">
          <small class="text-muted"><i class="bi bi-info-circle me-1"></i>
            El jefe de obra accede en: <code><?= (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off' ? 'https' : 'http').'://'.$_SERVER['HTTP_HOST'].'/jefe_obra_login.php' ?></code>
          </small>
        </div>
      </div>
    </div>
  ...
  <!-- Modal crear/editar Jefe de Obra -->
  <div class="modal fade" id="modalAddJefeObra" tabindex="-1">
    <div class="modal-dialog">
      <div class="modal-content">
        <form method="POST">
          <div class="modal-header text-white" style="background:#15803d">
            <h5 class="modal-title" id="jefeModalTitulo"><i class="bi bi-shield-fill-check me-2"></i>Nuevo Jefe de Obra</h5>
            <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
          </div>
          <div class="modal-body">
            <input type="hidden" name="accion" id="jefeAccion" value="crear_jefe_obra">
            <input type="hidden" name="id" id="jefeId" value="">
            <div class="row g-2">
              <div class="col-md-6"><label class="form-label small">Nombre *</label><input type="text" name="nombre" id="jefeNombre" class="form-control" required></div>
              <div class="col-md-6"><label class="form-label small">RUT</label><input type="text" name="rut" id="jefeRut" class="form-control"></div>
              <div class="col-md-12">
                <label class="form-label small">Obra asignada</label>
                <select name="obra_id" id="jefeObra" class="form-select">
              ...
              <div class="col-md-7"><label class="form-label small">Email</label><input type="email" name="email" id="jefeEmail" class="form-control"></div>
              <div class="col-md-5"><label class="form-label small">Teléfono</label><input type="text" name="telefono" id="jefeTel" class="form-control"></div>
              <div class="col-md-6"><label class="form-label small">Usuario *</label><input type="text" name="usuario" id="jefeUsu" class="form-control" required></div>
              <div class="col-md-6"><label class="form-label small">Contraseña <span id="jefePassNote" class="text-muted small">(*)</span></label><input type="text" name="password" id="jefePass" class="form-control"></div>
            </div>
          </div>
          <div class="modal-footer">
            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
            <button type="submit" class="btn text-white" style="background:#15803d"><i class="bi bi-check-lg me-1"></i>Guardar</button>
          </div>
        </form>
      </div>
    </div>
...
/* ── Jefe de Obra ── */
function editJefeObra(jo) {
  document.getElementById('jefeAccion').value = 'editar_jefe_obra';
  document.getElementById('jefeId').value     = jo.id;
  document.getElementById('jefeNombre').value = jo.nombre || '';
  document.getElementById('jefeRut').value    = jo.rut    || '';
  document.getElementById('jefeEmail').value  = jo.email  || '';
  document.getElementById('jefeTel').value    = jo.telefono || '';
  document.getElementById('jefeUsu').value    = jo.usuario || '';
  document.getElementById('jefeObra').value   = jo.obra_id || '';
  document.getElementById('jefePass').value   = '';
  document.getElementById('jefePass').required = false;
  document.getElementById('jefePassNote').textContent = '(dejar vacío para no cambiar)';
  document.getElementById('jefeModalTitulo').innerHTML = '<i class="bi bi-shield-fill-check me-2"></i>Editar Jefe — ' + jo.nombre;
  new bootstrap.Modal(document.getElementById('modalAddJefeObra')).show();
}
document.getElementById('modalAddJefeObra')?.addEventListener('show.bs.modal', function(e) {
  if (!e.relatedTarget) return;
  document.getElementById('jefeAccion').value = 'crear_jefe_obra';
  document.getElementById('jefeId').value     = '';
  ['jefeNombre','jefeRut','jefeEmail','jefeTel','jefeUsu','jefePass'].forEach(function(id){ document.getElementById(id).value = ''; });
  document.getElementById('jefeObra').value = '';
  document.getElementById('jefePass').required = true;
  document.getElementById('jefePassNote').textContent = '(*)';
  document.getElementById('jefeModalTitulo').innerHTML = '<i class="bi bi-shield-fill-check me-2"></i>Nuevo Jefe de Obra';
});
...
function abrirQRModal(ppuId, ppuLabel, desc, m3) {
  ...
  // ← URL con texto de patente (no ID numérico) para que el escáner lo lea correctamente
  var url     = baseUrl + '/validar_ppu.php?ppu=' + encodeURIComponent(ppuLabel);
