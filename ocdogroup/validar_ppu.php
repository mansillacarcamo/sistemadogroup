...
$ppuId = (int)($_GET['ppu'] ?? 0);
if (!$ppuId) { header('Location: despacho_externo_login.php'); exit; }

/* Datos de la PPU */
$stPpu = $pdo->prepare("SELECT * FROM despacho_ppu WHERE id=? AND activo=1");
$stPpu->execute([$ppuId]);
$ppuRow = $stPpu->fetch(PDO::FETCH_ASSOC);
if (!$ppuRow) { header('Location: despacho_externo_login.php'); exit; }
...
<div class="container py-4" style="max-width:640px;">
  ...
  <p class="text-center text-muted mt-4 small">DOGROUP &copy; <?= date('Y') ?> — Bynari SpA</p>
</div>
