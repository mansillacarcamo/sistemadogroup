document.addEventListener('DOMContentLoaded', function() {
  ...
  // Cantidad: NO tratar como moneda. Si hay coma, ésa es decimal y los puntos son miles.
  // Si no hay coma, el punto se interpreta como decimal (notación inglesa) — útil para
  // que el usuario pueda escribir "1.5" sin que el JS lo convierta en 15.
  function getNumCant(el) {
    if (!el) return 0;
  ...
  function recalcular() {
    ...
    document.querySelectorAll('.item-row').forEach(function(row) {
      var cant   = getNumCant(row.querySelector('.item-cant'));
      var precio = getNum(row.querySelector('.item-precio'));
      var dcto   = getNumCant(row.querySelector('.item-dcto'));
      var sub    = cant * precio;
  ...
  function actualizarMoneda() {
    ...
    document.querySelectorAll('.item-precio').forEach(function(el) {
      var n = getNum(el);
      el.value = n === 0 ? '0' : formatInput(n);
    });
    recalcular();
  ...
  if (btnAdd) {
    btnAdd.addEventListener('click', function(ev) {
      ...
      // Si algún otro listener intenta robar el focus en los próximos 250ms,
      // lo redirigimos de vuelta al campo Cantidad.
