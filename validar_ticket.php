...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['validar'])) {
  ...
  if (!$tok) { $error = 'Token inválido.'; }
    ...
    $stUp = $pdo->prepare("UPDATE tickets_despacho SET estado='validado', validado_en=datetime('now','localtime'), validado_por=?, validado_por_nombre=? WHERE token=? AND estado='enviado'");
